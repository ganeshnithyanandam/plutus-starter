...
payRewards = do
        ...
        os  <- map snd . Map.toList <$> utxosAt (oracleAddress orcl)
        let markerUtxo = Map.filter (\x -> csMatcher (markerCurSymbol tn') $ view ciTxOutValue x) scrUtxos
            markerAdaVal = toValue . fromValue $ mconcat [view ciTxOutValue o | o <- map snd $ Map.toList markerUtxo]
            markerVal = mconcat [view ciTxOutValue o | o <- map snd $ Map.toList markerUtxo, csMatcher (markerCurSymbol tn') (view ciTxOutValue o) == True]
            val = mconcat [view ciTxOutValue o | o <- os, csMatcher (markerCurSymbol tn') (view ciTxOutValue o) == False]
            lookups = Constraints.typedValidatorLookups (oracleScriptInst orcl)
                      <> Constraints.unspentOutputs scrUtxos
            constraints = collectFromScript scrUtxos (Update)
                          <> Constraints.mustPayToPubKey (pkh') val
                          <> Constraints.mustPayToPubKey (ownPkh) markerAdaVal
